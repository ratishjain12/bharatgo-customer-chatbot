...
export default function Chatbot({ embedded = false, onClose }: { embedded?: boolean; onClose?: () => void }) {
  ...
  const sendMessage = async (text: string, opts?: { addUserBubble?: boolean }) => {
    ...
    try {
      ...
      if (res.requiresUserInfo) {
        ...
        return; // wait for user info, then we'll resume
      }
  ...
  const handleUserModalSubmit = (e: React.FormEvent) => {
    ...
    try {
      ...
      // If there was a pending question, resend it without duplicating the user bubble
      if (pendingQuestion) {
      ...
      >
        {messages.length === 0 ? (
          ...
          >
            ...
            >
              {["Where is my order?", "What is the return/refund policy?", "What is the shipping policy?"].map((q) => (
                <button
                  key={q}
                  type="button"
                  disabled={isLoading}
